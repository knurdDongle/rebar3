...
-callback metadata(rebar_app_info:t()) -> #{src_dirs     => [file:dirname()],
...
run(CompilerMod, AppInfo) ->
    ...
    #{src_dirs := SrcDirs,
      ...
      src_ext := SrcExt,
      out_mappings := Mappings} = CompilerMod:metadata(AppInfo),
...
clean(Compilers, AppInfo) ->
    ...
    lists:foreach(fun(CompilerMod) ->
                          #{src_dirs := SrcDirs,
                            src_ext := SrcExt} = CompilerMod:metadata(AppInfo),
                          FoundFiles = find_source_files(BaseDir, SrcExt, SrcDirs, Opts),
                          CompilerMod:clean(FoundFiles, AppInfo),
                          rebar_file_utils:rm_rf(dag_file(CompilerMod, OutDir))
                  end, Compilers).
...
modify_dag(G, Compiler, Source, LastModified, Dir, Dirs) ->
    AbsIncls = Compiler:source_dependencies(Source, Dir, Dirs),
